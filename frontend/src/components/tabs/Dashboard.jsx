  ...
  return (
    <div>
      ...
      {isGuest ? (
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl p-5 text-center">
            <div className="text-3xl font-bold text-orange-700 mb-1">{stats.totalOptimizations}</div>
            <div className="text-xs text-stone-400 dark:text-zinc-500">optimizasyon yapıldı</div>
          </div>
          <div className="bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl p-5 text-center">
            <div className="text-3xl font-bold text-orange-700 mb-1">{stats.totalSavedTokens > 0 ? stats.totalSavedTokens.toLocaleString() : '-'}</div>
            <div className="text-xs text-stone-400 dark:text-zinc-500">token tasarruf edildi</div>
          </div>
          <div className="bg-white dark:bg-zinc-900 border border-stone-200 dark:border-zinc-700 rounded-2xl p-5 text-center">
            <div className="text-3xl font-bold text-orange-700 mb-1">{stats.avgPercentage > 0 ? `%${stats.avgPercentage}` : '-'}</div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {TABS
            ...
            .map(tab => (
              ...
              >
                <div className="text-orange-700 mb-3 group-hover:scale-110 transition-transform inline-block">
                  {tab.icon}
                </div>
      ...
      <div className="bg-orange-50 dark:bg-orange-500/10 border border-orange-100 dark:border-orange-500/20 rounded-2xl p-4 flex items-start gap-3">
        <LightningIcon size={16} className="text-orange-700 shrink-0 mt-0.5" weight="regular" />
        <div>
          <span className="text-xs font-medium text-orange-700 dark:text-orange-400 tracking-wider">İPUCU</span>
